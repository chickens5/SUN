**Gabe's Geomagnetic Storm Prediction Pipeline (GGSP)**

**UMSL · Class of 2026 · Computer Science · chickens5**

GGSSP-7.0.py uses our ggsp_pipeline to predict the **planetary Kp index** (a scale of geomagnetic storm intensity) from real-time
...
| File | Purpose |
|------|---------|
| `ggsp_pipeline_v7.py` | Core data pipeline â€” fetch, features, model, forecast |
...
All sources are public no API key required. Data from DSCOVR at the L1 Lagrange point (~1.5M km sunward) gives 30â€“60 minutes of warning before solar wind reaches Earth.
...
| `density_mean` | Proton number density â€” sets dynamic pressure |
| `by_mean` | IMF By component â€” sets clock angle Î¸ in Newell coupling |
| `bz_mean` | Southward IMF (negative = direct field reconnection) |
| `bz_min` | Extreme southward excursion â€” storms are triggered by *bursts* |
| `bt_mean` | Total IMF magnitude â€” reconnection rate âˆ Bt^(2/3) |
| `coupling_mean` | **Newell coupling function** (see below) |
| `p_dyn_mean` | Dynamic ram pressure = density Ã— (speed/100)Â² â€” CME sudden commencement |
| `kp_lag_3h` | Kp 3 hours ago â€” storm onset/recovery memory |
| `kp_lag_6h` | Kp 6 hours ago |
| `kp_lag_9h` | Kp 9 hours ago |

### Newell Coupling Function
*Newell et al. (2007), JGR 112, A01206 â€” the core physics feature:*
...
in the model (~40â€“45% of total feature importance).
...
the clock-angle term during CMEs and CIRs â€” exactly the events we care most about.
...
It handles the nonlinear solar windâ€“magnetosphere relationship without manual exponent tuning
...
**Train/test split:** Strictly chronological (75% train / 25% test). No shuffling â€” shuffling
...
**Current performance** (1990â€“2026, ~84k 3h samples):

| Metric | Value | Context |
|--------|-------|---------|
| MAE | **0.440 Kp** | Below the ~0.3 Kp measurement noise floor of the index itself |
| RÂ² | **0.782** | NOAA operational products typically score 0.60â€“0.75 |
| Baseline MAE | 1.051 Kp | "Always predict the mean" â€” model beats this by 58% |
...
|----------|--------|----------------------|
| **Quiet** | 20% | Declining speed, near-zero Bz â€” post-storm recovery |
| **Moderate** | 50% | Average wind with occasional Bz dips â€” most common regime |
| **Active** | 30% | Fast wind (CME/CIR), sustained southward Bz â€” storm conditions |

Each scenario generates synthetic solar wind for 24 Ã— 3-hour steps (72 hours total).
`predict_scenario_kp()` runs an **autoregressive loop** â€” each predicted Kp feeds back
...
```
Kp_weighted[t] = 0.20 Ã— Kp_Quiet[t] + 0.50 Ã— Kp_Moderate[t] + 0.30 Ã— Kp_Active[t]
```
...
**Why it matters:** Near solar maximum (SSN ~150+), CMEs and CIRs occur ~5x— more often
than at solar minimum. The fixed scenario weights (Quiet 20% / Moderate 50% / Active 30%)
do not reflect this â€” during SC25 peak, the Active scenario deserves more probability mass.

**Data:** SIDC International Sunspot Number v2.0 (official global standard since 2015).

**Features:** 7 autoregressive lags (1â€“36 months) + 12-month rolling mean + Fourier terms
at the 11-year and 5.5-year harmonics (absolute epoch so phase is always physically correct).

**Output â€” `ggsp_integration` block:**
```json
{
...
- Newell et al. (2007). *A nearly universal IMFâ€“magnetosphere coupling function.* JGR 112, A01206.
...
*Last updated: May 2026 â€” chickens5 | UMSL CS 2026*
